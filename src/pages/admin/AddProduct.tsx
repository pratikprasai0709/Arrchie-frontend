...
import { PlusCircle, ClipboardCheck, ArrowLeft, RefreshCw, Sparkles, Upload, X } from 'lucide-react';
...
export default function AddProduct() {
  ...
  // Suggested curated Unsplash templates for quick selection when adding flask catalogs
  const templates = [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !description || !price || !brand || !capacity) {
      setErr('Please fill in all mandatory product catalog fields.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setErr(data.message || 'Could not instantiate product catalog item.');
        setLoading(false);
        return;
      }
      ...
      setErr('A connection error occurred. Please try again.');
      setLoading(false);
    }
  };
      ...
      <main className="flex-1 p-6 sm:p-10 space-y-10 overflow-y-auto">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 font-mono">
            Register a premium, sustainable flask item in the store catalog.
          </p>
        </div>

        {success && (
          <div className="p-4 bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 text-xs font-semibold rounded-xl flex items-center gap-2">
            <ClipboardCheck className="w-5 h-5" />
            Flask registered successfully! Redirecting back to catalog list...
          </div>
        )}

        {err && (
          <div className="p-4 bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-semibold rounded-xl">
            ❌ {err}
          </div>
        )}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5 col-span-1 md:col-span-2">
              ...
              <input
                ...
                placeholder="AeroHydro Matte Pure Gold"
            ...
            <div className="space-y-1.5 col-span-1 md:col-span-2">
              ...
              <textarea
                ...
                placeholder="Describe material insulation layers, BPA testing specs, lifetime warranty promises..."
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="HydroFlow or AeroActive"
                className="w-full px-3.5 py-2.5 text-sm bg-slate-950 text-white border border-slate-800 rounded-xl focus:border-amber-500 focus:outline-hidden transition-all"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 text-sm bg-slate-950 text-white border border-slate-800 rounded-xl focus:border-amber-500 focus:outline-hidden transition-all"
              >
                <option value="500ml">500 ml size</option>
            ...
            <div className="col-span-1 md:col-span-2 pt-6">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 px-6 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-bold font-sans shadow-lg shadow-amber-500/5 active:scale-[0.98] transition-all cursor-pointer"
              >
                {loading ? 'Building product catalog entry...' : 'Confirm Registration to Store'}
              </button>
            </div>
